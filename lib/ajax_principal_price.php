...
if(isset($_POST['zone_id_check'])){
    $country_check = $_POST['zone_id_check'];
    $principal_price_id_check = $_POST['principal_price_id_check'];
    
    $i=1;
    $j=1;
    
    $query_p = "SELECT * FROM principal_price WHERE principal_id='$principal_price_id_check' AND zone='$country_check' AND goods_type='P'";
    $result_p = $db->link->query($query_p);
    if($result_p->num_rows > 0){
        $i=0;
    }
    
    $query_d = "SELECT * FROM principal_price WHERE principal_id='$principal_price_id_check' AND zone='$country_check' AND goods_type='D'";
...
if(isset($_POST['client_email'])){
    ...
    while($row = $result->fetch_assoc()){
        echo '<tr class="center">';
        echo '<td class="center">'.$i.'</td><td>'.$row['goods_type'].'</td><td>'.$row['weight'].'</td><td>'.$row['price'].'</td><td></td>';
        echo '</tr>';
        $i=$i+1;
    }
