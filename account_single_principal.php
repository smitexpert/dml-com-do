                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group connected-group">
                                                    ...
                                                    <select name="CorpoClient" required id="CorpoClient" class="form-control selectpicker" data-show-subtext="true" data-live-search="true">
                                                            <option value="">--</option>
                                                            <?php 
                                                                $selectcouriercomp = "SELECT cour_comp_id,cour_comp_name FROM tbl_courier_companies WHERE status=1";
                                                                 $findclientname =  $Courcompanyset->selectcourComp($selectcouriercomp);    
                                                            if ($findclientname) { while ($getcourcomp=$findclientname->fetch_assoc()) { ?>
                                                                <option id="courcompclass" class="courcompclass" value="<?php echo $getcourcomp['cour_comp_id']; ?>"><?php echo  $client_name = $getcourcomp['cour_comp_name']; ?></option>
...
cour_comp_name
    function getClientprices(){
        var cour_comp_id = $("#CorpoClient").val();
        var cour_comp_name= $('#CorpoClient option:selected').html();
        $("#corp_client").val(cour_comp_id);
        $("#corp_client").html(cour_comp_name);
        $("#clnamemdelhead").html(cour_comp_name);

        //alert(corp_client);exit();
           $.ajax({  
            url:"getCorpoClientPriceset.php",  
            method:"POST",  
            data:{action:'getgenpr',corp_client:corp_client,nameofclient:nameofclient},  
                //dataType: "JSON",
                success:function(data){  
