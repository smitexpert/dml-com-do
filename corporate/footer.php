		...
		<?php
		if ($uri_parts[0] == '/corporate/agent_client_view_price.php') {
			?>
			<script>
				$(".nav_view li").click(function() {
					$(".nav_view li").removeClass('active');
		...
		<?php
		if ($uri_parts[0] == '/corporate/search_price.php') {
			?>
			<script>
		...
		?>
		<?php
		if ($uri_parts[0] == '/corporate/booking.php') {
			?>
			<script>
				...
				function get_agent_price() {
					...
					if ((agent_dest_country != "") && (agent_goods_type != "") && (agent_goods_weight != "") && (agent_sender_mail != "")) {
		...
		<?php
		if ($uri_parts[0] == '/corporate/consignment_view.php') {
			?>
			<script>
		...
		?>
		<?php
		if ($uri_parts[0] == '/corporate/agent_cons_update.php') {
			?>
			<script>
				$(document).ready(function() {
		...
		<?php
		if ($uri_parts[0] == '/corporate/agent_accounts.php') {
			?>
			<script>
		...
		<?php
		if ($uri_parts[0] == '/corporate/agent_settings.php') {
			?>
			<script>
				$(".nav_view a").click(function() {
					var id = $(this).attr("id");

					$(".view-item").css("display", "none");
					$("#view_" + id).css("display", "block");
				});
				...
				$("#update_password").submit(function(event) {
					...
					//GET OLD PASS FROM DATABASE
					if (old_pass != "" && new_pass != "" && confirm_pass != "") {
						if(new_pass == confirm_pass){
							var con = confirm("Are you sure?");
						if (con == 1) {
							$.ajax({
								url: "ajax/ajax_agent_settings.php",
								method: "POST",
								data: {
									agent_old_pass_check: old_pass,
									agent_email: agent_email,
									new_pass: new_pass,
						...
						alert("Plaese fill up all* required field");
					}

					// console.log("old_pass :" + agent_email);
					// console.log("new_pass :" + agent_id);
