...
function corporate_login($table_id, $password, $client_id){
    Session::checkLogin();
    $password = md5($password);
    $db = new Database();
    
    $sql = "SELECT * FROM corporate_clients WHERE id='$table_id' AND password='$password' AND status='1'";
    $query = $db->link->query($sql);
    if($query->num_rows > 0){
        while($row = $query->fetch_assoc()){
            Session::set('corporate_login', true);
            Session::set('corporate_id', $client_id);
            Session::set('client_table_id', $row['id']);
            Session::set('corporate_email', $row['email']);
            Session::set('corporate_name', $row['name']);
            Session::set('corporate_company', $row['company_name']);
            Session::set('contact', $row['contact']);
            Session::set('address', $row['address']);
            Session::set('type', "corporate");
        }
        header('Location: /corporate/');
    }else{
        return "0";
    }
}
...
function client_login($client_id, $password){
    ...
    if($query->num_rows > 0){
        ...
        if($row['client_type'] == 'agent'){
            ...
            $table_id = $row['table_id'];
            if(corporate_login($table_id, $password, $client_id) == '0'){
                return "0";
            }
        }
    }else{
