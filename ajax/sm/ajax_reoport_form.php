...
if(isset($_POST['app_status'])){
    ...
    if(($app_status != '1') && ($app_report != "")){
        ...
        $get = "SELECT * FROM marketing_appointment_plan_report WHERE app_plan_id='$app_id'";
        $get_query = $db->link->query($get);

        if($get_query->num_rows > 0){
            $sql = "UPDATE marketing_appointment_plan_report SET report='$app_report' WHERE app_plan_id='$app_id'; UPDATE marketing_appointment_plan SET status='$app_status' WHERE id='$app_id'";
...
if(isset($_POST['visit_status'])){
    ...
    if(($visit_status != "1") && ($visit_report != "")){
        ...
        $get = "SELECT * FROM marketing_add_plan_report WHERE visit_plan_id='$visit_plan_id'";
        $get_query = $db->link->query($get);
        if($get_query->num_rows > 0){
            $sql = "UPDATE marketing_add_plan_report SET report='$visit_report' WHERE visit_plan_id='$visit_plan_id'; UPDATE marketing_add_plan SET status = '$visit_status' WHERE id='$visit_plan_id'";
